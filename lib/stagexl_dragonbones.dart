library stagexl_dragonbones;

import 'dart:convert';
import 'dart:math' as math;
import 'dart:typed_data';
import 'package:stagexl/stagexl.dart';

part 'src/data/animation.dart';
part 'src/data/animation_object.dart';
part 'src/data/animation_object_frame.dart';
part 'src/data/armature.dart';
part 'src/data/display.dart';
part 'src/data/skin.dart';
part 'src/data/skin_slot.dart';

part 'src/data/bone/bone.dart';
part 'src/data/bone/bone_animation.dart';
part 'src/data/bone/bone_animation_frame.dart';
part 'src/data/mesh/mesh.dart';
part 'src/data/mesh/mesh_animation.dart';
part 'src/data/mesh/mesh_animation_frame.dart';
part 'src/data/slot/slot.dart';
part 'src/data/slot/slot_animation.dart';
part 'src/data/slot/slot_animation_frame.dart';

part 'src/math/curve.dart';
part 'src/math/tools.dart';
part 'src/math/transform.dart';

part 'src/skeleton/skeleton.dart';
part 'src/skeleton/skeleton_object.dart';
part 'src/skeleton/skeleton_object_animation.dart';

part 'src/skeleton/bone/skeleton_bone.dart';
part 'src/skeleton/bone/skeleton_bone_animation.dart';
part 'src/skeleton/display/skeleton_display.dart';
part 'src/skeleton/display/skeleton_display_armature.dart';
part 'src/skeleton/display/skeleton_display_image.dart';
part 'src/skeleton/display/skeleton_display_mesh.dart';
part 'src/skeleton/display/skeleton_mesh_animation.dart';
part 'src/skeleton/slot/skeleton_slot.dart';
part 'src/skeleton/slot/skeleton_slot_animation.dart';

part 'src/dragon_bones.dart';
part 'src/dragon_bones_parser_json4.dart';
