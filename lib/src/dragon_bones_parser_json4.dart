...
class _DragonBonesParserJson4 {
  ...
  static Bone _parseBone(Map data) {
    return Bone(_getString(data, 'name', ''), _getString(data, 'parent', ''),
        _getInt(data, 'length', 0), _getTransform(data, 'transform'));
  }

  static Slot _parseSlot(Map data) {
    return Slot(_getString(data, 'name', ''), _getString(data, 'parent', null),
        _getInt(data, 'displayIndex', 0), _getInt(data, 'z', 0));
  }

  static Skin _parseSkin(Map data) {
    return Skin(_getString(data, 'name', ''),
        _getList<SkinSlot>(data, 'slot', _parseSkinSlot));
  }

  static SkinSlot _parseSkinSlot(Map data) {
    return SkinSlot(_getString(data, 'name', ''),
  ...
  static SlotAnimationFrame _parseAnimationSlotFrame(Map data) {
    ...
    return SlotAnimationFrame(
        duration, curve, easing, colorTransform, displayIndex, zOrder);
  }
  ...
  static Transform _getTransform(Map data, String key) {
    var value = data.containsKey(key) ? data[key] : {};
  ...
  static List<T> _getList<T>(Map<String, dynamic> data, String key, parser) {
    var value = data.containsKey(key) ? data[key] as List : [];
  ...
  static Int16List _getInt16List(Map data, String key, parser) {
    var value = data.containsKey(key) ? data[key] as List : [];
    var list =
        value.map<int>((result) => parser(result)).toList(growable: false);
  ...
  static Float32List _getFloat32List(Map data, String key, parser) {
    var value = data.containsKey(key) ? data[key] as List : [];
    var list =
        value.map<double>((result) => parser(result)).toList(growable: false);
